...
class DialogSystem:
    ...
    def get_intent(self, memory):
        ...
        for node_id in memory["available_nodes"]:
            ...
            if score > max_score:
                ...
                memory['current_score'] = max_score
    ...
    def get_slot(self, memory):
        ...
        for slot in self.node_id_to_node_info[current_node].get("slot", []):
    ...
    def dst(self, memory):
        ...
        current_node = memory["current_node"]
        slots = self.node_id_to_node_info[current_node].get("slot", [])
        for slot in slots:
    ...
    def policy(self, memory):
        ...
        if memory["need_slot"] is None:
            ...
            #开放子节点
            memory["available_nodes"] = self.node_id_to_node_info[memory["current_node"]].get("childnode", [])
            #执行动作
            # self.take_action(memory)
        else:
            memory["action"] = "ask"
            #停留在当前节点
            memory["available_nodes"] = [memory["current_node"], 'scenario-买衣服_node5']
        return memory
    ...
    def nlg(self, memory):
        ...
        if memory["action"] == "answer":
            #直接回答
            answer = self.node_id_to_node_info[memory["current_node"]]["response"]
